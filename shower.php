...
$sql = "SELECT id FROM owneraccounts WHERE 
        umobilenumber = '$mobilenumber' AND 
        smobilenumber = '$smobilenumber' AND 
        purchasedate = '$purchasedate' AND 
        balance = '$balance' AND 
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        $id = $row['id'];
        // Now, $id contains the value of the 'id' column based on the specified conditions
    }
}
$sql = "SELECT * FROM owneraccounts 
WHERE umobilenumber='$mobilenumber' LIMIT 1";
$result = $conn->query($sql);
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
                ...
                echo'  <ul class="list-group">';
                   echo' <li class="list-group-item border-0 d-flex p-4 mb-2 bg-gray-100 border-radius-lg">';
                      echo'<div class="d-flex flex-column">';
                        ...
                        echo "<h6 class='mb-3 text-center text-lg'>&nbsp;&nbsp;&nbsp;" .$row['username']. "</h6>";
           
                        //echo "<h6 class='mb-2 text-sm'>User Mobilenumber: <span class='text-dark font-weight-bold ms-sm-2'>" .$row['umobilenumber']. "</span></h6>";
                        if (!empty($credit)) {
                          echo "<h6 class='mb-2 text-sm'>Bill Amount: <span class='text-dark ms-sm-2 font-weight-bold'>" .$credit. "</span></h6>";
                        }
                        if(!empty($debit)){
                          echo "<h6 class='mb-2 text-sm'>Amount Paid: <span class='text-dark ms-sm-2 font-weight-bold'>" .$debit. "</span></h6>";
                        }
                        if(!empty($balance)){
            ...
            if (!empty($row['notes'])) {
             ...
             echo' <div class="ms-auto text-end">';
                       // echo' <a class="btn btn-link text-danger text-gradient px-3 mb-0" href="edit.php?username='.$row['username'].'&mobilenumber='.$row['umobilenumber'].'&credit='.$credit.'&debit='.$debit.'&balance ='.$balance.'&itemscount='.$itemscount.'&notes='.$row['notes'].'&id='.$row['id'].'"><i class="fas fa-pencil-alt text-dark me-2" aria-hidden="true"></i>Edit</a>';
                  echo'</div>';
                echo'</li>';
             echo' </ul>';
           echo' </div>';
